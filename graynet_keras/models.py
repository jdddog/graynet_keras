...
def RESNET50_LMIC(input_tensor, w_reg, SEPARATE_ACT=False, return_tensor=False):
    ...
    def conv_block(input_tensor, kernel_size, filters, stage, block, strides=(2, 2), w_reg=None):
        ...
        x = Conv2D(filters1, (1, 1), strides=strides, name=conv_name_base + "2a", kernel_regularizer=w_reg)(
            input_tensor)
        x = BatchNormalization(axis=bn_axis, name=bn_name_base + "2a")(x)
        x = Activation("relu", name=relu_name_base + "_branch2a_relu")(x)
        ...
        shortcut = Conv2D(filters3, (1, 1), strides=strides, name=conv_name_base + "1", kernel_regularizer=w_reg)(
            input_tensor)
...
def INCEPTIONV3_LMIC(input_tensor, w_reg, SEPARATE_ACT=False, return_tensor=False):
    def conv2d_bn(x, filters, num_row, num_col, padding="same", strides=(1, 1), w_reg=None, name=""):
        ...
        x = Conv2D(filters, (num_row, num_col), strides=strides, padding=padding, use_bias=False,
                   kernel_regularizer=w_reg, name=conv_name)(x)
    ...
    branch3x3dbl = conv2d_bn(branch3x3dbl, 96, 3, 3, w_reg=w_reg,
                             name="mixed0_brnch3x3dbl_blk1")  # 64x64x64 -> 64x64x96
    branch3x3dbl = conv2d_bn(branch3x3dbl, 96, 3, 3, w_reg=w_reg,
                             name="mixed0_brnch3x3dbl_blk2")  # 64x64x96 -> 64x64x96

    branch_pool = AveragePooling2D((3, 3), strides=(1, 1), padding="same", name="mixed0_brnchpool_blk0_pool")(
        x)  # 64x64x192 -> 64x64x192
    ...
    branch3x3dbl = conv2d_bn(branch3x3dbl, 96, 3, 3, w_reg=w_reg,
                             name="mixed1_brnch3x3dbl_blk1")  # 64x64x64 -> 64x64x96
    branch3x3dbl = conv2d_bn(branch3x3dbl, 96, 3, 3, w_reg=w_reg,
                             name="mixed1_brnch3x3dbl_blk2")  # 64x64x96 -> 64x64x96

    branch_pool = AveragePooling2D((3, 3), strides=(1, 1), padding="same", name="mixed1_brnchpool_blk0_pool")(
        x)  # 64x64x256 -> 64x64x256
    ...
    branch3x3dbl = conv2d_bn(branch3x3dbl, 96, 3, 3, w_reg=w_reg,
                             name="mixed2_brnch3x3dbl_blk1")  # 64x64x64 -> 64x64x96
    branch3x3dbl = conv2d_bn(branch3x3dbl, 96, 3, 3, w_reg=w_reg,
                             name="mixed2_brnch3x3dbl_blk2")  # 64x64x96 -> 64x64x96

    branch_pool = AveragePooling2D((3, 3), strides=(1, 1), padding="same", name="mixed2_brnchpool_blk0_pool")(
        x)  # 64x64x288 -> 64x64x288
    ...
    # -------------------------------------
    branch3x3 = conv2d_bn(x, 384, 3, 3, strides=(2, 2), w_reg=w_reg,
                          name="mixed3_brnch3x3_blk0")  # 64x64x288 -> 32x32x384

    branch3x3dbl = conv2d_bn(x, 64, 1, 1, w_reg=w_reg, name="mixed3_brnch3x3dbl_blk0")  # 64x64x288 -> 64x64x64
    branch3x3dbl = conv2d_bn(branch3x3dbl, 96, 3, 3, w_reg=w_reg,
                             name="mixed3_brnch3x3dbl_blk1")  # 64x64x64 -> 64x64x96
    branch3x3dbl = conv2d_bn(branch3x3dbl, 96, 3, 3, strides=(2, 2), w_reg=w_reg,
                             name="mixed3_brnch3x3dbl_blk2")  # 64x64x96 -> 32x32x96

    branch_pool = MaxPooling2D((3, 3), strides=(2, 2), padding="same", name="mixed3_brnchpool_blk0_pool")(
        x)  # 64x64x288 -> 32x32x288
    ...
    branch7x7dbl = conv2d_bn(branch7x7dbl, 128, 7, 1, w_reg=w_reg,
                             name="mixed4_brnch7x7dbl_blk1")  # 32x32x128 -> 32x32x128
    branch7x7dbl = conv2d_bn(branch7x7dbl, 128, 1, 7, w_reg=w_reg,
                             name="mixed4_brnch7x7dbl_blk2")  # 32x32x128 -> 32x32x128
    branch7x7dbl = conv2d_bn(branch7x7dbl, 128, 7, 1, w_reg=w_reg,
                             name="mixed4_brnch7x7dbl_blk3")  # 32x32x128 -> 32x32x128
    branch7x7dbl = conv2d_bn(branch7x7dbl, 192, 1, 7, w_reg=w_reg,
                             name="mixed4_brnch7x7dbl_blk4")  # 32x32x128 -> 32x32x192

    branch_pool = AveragePooling2D((3, 3), strides=(1, 1), padding="same", name="mixed4_brnchpool_blk0_pool")(
        x)  # 32x32x768 -> 32x32x768
    ...
    for i in range(2):
        ...
        branch_pool = AveragePooling2D((3, 3), strides=(1, 1), padding="same",
                                       name="mixed{}_brnchpool_blk0_pool".format(num))(x)
    ...
    branch7x7dbl = conv2d_bn(branch7x7dbl, 192, 7, 1, w_reg=w_reg,
                             name="mixed7_brnch7x7dbl_blk1")  # 32x32x192 -> 32x32x192
    branch7x7dbl = conv2d_bn(branch7x7dbl, 192, 1, 7, w_reg=w_reg,
                             name="mixed7_brnch7x7dbl_blk2")  # 32x32x192 -> 32x32x192
    branch7x7dbl = conv2d_bn(branch7x7dbl, 192, 7, 1, w_reg=w_reg,
                             name="mixed7_brnch7x7dbl_blk3")  # 32x32x192 -> 32x32x192
    branch7x7dbl = conv2d_bn(branch7x7dbl, 192, 1, 7, w_reg=w_reg,
                             name="mixed7_brnch7x7dbl_blk4")  # 32x32x192 -> 32x32x192

    branch_pool = AveragePooling2D((3, 3), strides=(1, 1), padding="same", name="mixed7_brnchpool_blk0_pool")(
        x)  # 32x32x768 -> 32x32x768
    ...
    branch3x3 = conv2d_bn(branch3x3, 320, 3, 3, strides=(2, 2), w_reg=w_reg,
                          name="mixed8_brnch3x3_blk1")  # 32x32 -> 14x14x320
    ...
    branch7x7x3 = conv2d_bn(branch7x7x3, 192, 3, 3, strides=(2, 2), w_reg=w_reg,
                            name="mixed8_brnch7x7x3_blk3")  # 32x32 -> 14x14x192

    branch_pool = MaxPooling2D((3, 3), strides=(2, 2), padding="same", name="mixed8_brnchpool_blk0_pool")(
        x)  # 32x32 -> 14x14x768
    ...
    for i in range(2):
        ...
        branch3x3 = layers.concatenate([branch3x3_1, branch3x3_2], axis=channel_axis,
                                       name="mixed{}_brnch3x3_blk3_concat".format(num))
        ...
        branch3x3dbl = layers.concatenate([branch3x3dbl_1, branch3x3dbl_2], axis=channel_axis,
                                          name="mixed{}_brnch3x3dbl_blk4_concat".format(num))
...
def INCEPTION_RESNET_V2_LMIC(input_tensor, w_reg, SEPARATE_ACT=False, return_tensor=False):
    def conv2d_bn(x, filters, kernel_size, strides=1, padding="same", activation="relu", use_bias=False, w_reg=None,
                  name=None):

        ### Convolution
        conv_name = name + "_conv"
        x = Conv2D(filters, kernel_size, strides=strides, padding=padding, use_bias=use_bias, kernel_regularizer=w_reg,
                   name=conv_name)(x)
    ...
    def inception_resnet_block(x, scale, block_type, block_idx, w_reg=None, activation="relu"):
        if block_type == "block35":
            ...
            branch_1 = conv2d_bn(branch_1, 160, [1, 7], w_reg=w_reg,
                                 name="{}_{}_brnch1_blk1".format(block_type, block_idx))
            branch_1 = conv2d_bn(branch_1, 192, [7, 1], w_reg=w_reg,
                                 name="{}_{}_brnch1_blk2".format(block_type, block_idx))
            branches = [branch_0, branch_1]
        elif block_type == "block8":
            branch_0 = conv2d_bn(x, 192, 1, w_reg=w_reg, name="{}_{}_brnch0_blk0".format(block_type, block_idx))
            branch_1 = conv2d_bn(x, 192, 1, w_reg=w_reg, name="{}_{}_brnch1_blk0".format(block_type, block_idx))
            branch_1 = conv2d_bn(branch_1, 224, [1, 3], w_reg=w_reg,
                                 name="{}_{}_brnch1_blk1".format(block_type, block_idx))
            branch_1 = conv2d_bn(branch_1, 256, [3, 1], w_reg=w_reg,
                                 name="{}_{}_brnch1_blk2".format(block_type, block_idx))
            branches = [branch_0, branch_1]
        ...
        up = conv2d_bn(mixed, K.int_shape(x)[channel_axis], 1, activation=None, use_bias=True, w_reg=w_reg,
                       name=mixed_name)
    ...
    branch_pool = AveragePooling2D(3, strides=1, padding="same", name="mixed5b_brnchpool_blk0_pool")(
        x)  # 64x64x192 -> 64x64x192
    ...
    branch_pool = MaxPooling2D(3, strides=2, padding="same", name="mixed6a_brnchpool_blk0_pool")(
        x)  # 64x64x320 -> 32x32x320
    ...
    branch_pool = MaxPooling2D(3, strides=2, padding="same", name="mixed7a_brnchpool_blk0_pool")(
        x)  # 32x32x1088 -> 16x16x1088
